/**
 * POST /api/dashboard/dj/mixes/canvas/generate
 *
 * Triggers a Remotion Lambda render of the TrackCanvas composition for a DJ mix,
 * polls for completion, saves the output URL, and sends an in-app notification.
 *
 * Body:    { mixId: string; sessionId?: string }
...
export async function POST(req: NextRequest) {
  ...
  let body: { mixId?: string; sessionId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const mix = await db.dJMix.findUnique({
    where: { id: mixId },
    select: { id: true, djProfileId: true, audioUrl: true, coverArtUrl: true },
  });
  ...
  if (sessionId) {
    ...
    if (
      checkoutSession.metadata?.mixId !== mixId ||
  ...
  const { renderId, bucketName } = await renderMediaOnLambda({
    ...
    framesPerLambda: 60,
    privacy: "public",
    outName: `canvas-mix-${mix.id}-${Date.now()}.mp4`,
  });
  ...
  void createNotification({
    ...
    title: "Your Mix Canvas is ready",
    message: "Your mix canvas video has been generated and is ready to use.",
    link: "/dashboard/dj/mixes",
  }).catch(() => {});
