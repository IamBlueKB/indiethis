 ...
 *
 * Replicate calls this when a revision mix pass completes (REVISING status).
 * Output shape is identical to the mix webhook. Stores updated file paths + waveforms,
 * then fires preview-mix so the artist can compare the revised version.
 */
...
export async function POST(req: NextRequest) {
  ...
  console.error("revise webhook body:", JSON.stringify(body).slice(0, 500));
  ...
  try {
    ...
    // Same output shape as the mix action
    ...
    // Fire preview-mix — same final step as initial mix
    ...
    console.error(`revise webhook failed for job ${jobId}:`, message);
    await prisma.mixJob.update({
      where: { id: jobId },
      data:  { status: "FAILED" },
    });
