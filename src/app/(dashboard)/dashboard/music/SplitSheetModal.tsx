    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <select
          ...
          disabled={!isCreating}
          className="rounded-lg border px-2.5 py-2 text-xs text-foreground outline-none focus:ring-1 focus:ring-accent/50 disabled:opacity-60"
          style={{ borderColor: "var(--border)", backgroundColor: "#1a1a1a", colorScheme: "dark" }}
        >
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => {
              ...
              if (raw === "" || raw === ".") {
                onChange(index, "percentage", 0);
              } else {
                const n = parseFloat(raw);
                if (!isNaN(n)) onChange(index, "percentage", n);
              }
            }}
            onBlur={(e) => {
              if (e.target.value === "") onChange(index, "percentage", 0);
            }}
            type="number"
            min={0}
            max={100}
