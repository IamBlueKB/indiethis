...
import {
  MapPin, Disc3, Radio, BookOpen, Podcast,
...
export default function DJBookingReportPage({
  searchParams,
}: {
  ...
  useEffect(() => {
    ...
    fetch("/api/dashboard/ai/booking-report?mode=DJ")
      .then((r) => r.json())
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-white">DJ Booking Opportunities</h1>
        <p className="text-sm text-neutral-400 mt-1">
          10 curated opportunities: club nights, festivals, radio slots, mix shows, and more — matched to your style and city.
        </p>
      </div>
      ...
      {(success || pending) && !report && (
        <div className="rounded-xl border border-[#D4A843]/30 bg-[#D4A843]/10 p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-[#D4A843]">
            We&apos;re finding DJ opportunities for you — check back in a moment.
          </p>
        </div>
      )}
      ...
      {!loading && !report && !pending && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-8 flex flex-col items-center text-center gap-5">
          <div className="w-14 h-14 rounded-full bg-[#D4A843]/15 flex items-center justify-center">
            <Disc3 size={26} className="text-[#D4A843]" />
          </div>
          <div>
            <h2 className="text-lg font-semibold text-white mb-1">Find Your Next Booking</h2>
            <p className="text-sm text-neutral-400 max-w-md">
              Get 10 curated DJ opportunities matched to your genre and city — club nights,
              festivals with DJ slots, radio guest mixes, podcasts, and more.
            </p>
          </div>
          <ul className="text-sm text-neutral-400 space-y-1 text-left w-full max-w-xs">
            {[
              "Club nights & venue residencies",
              "Festival DJ slots",
              "Radio guest mix shows",
              "DJ competitions & battles",
              "Music blogs & podcast features",
            ].map((item) => (
