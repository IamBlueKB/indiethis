...
/**
 * src/app/lyric-video/LyricVideoGateScreen.tsx
 *
 * Gate screen shown before the lyric video wizard.
 * Collects email + optional name via form or Google OAuth.
 * On submit: sets `indiethis_guest_email` cookie (7-day, JSON) then reloads.
 */

import { useState }  from "react";
import { useRouter } from "next/navigation";
import { signIn }    from "next-auth/react";
import { Mail, User, ArrowRight, Clapperboard, Sparkles, Film } from "lucide-react";

export default function LyricVideoGateScreen() {
  ...
  function handleGoogle() {
    signIn("google", { callbackUrl: "/lyric-video" });
  }

  function handleFacebook() {
    signIn("facebook", { callbackUrl: "/lyric-video" });
  }
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-black text-white">Create Your Lyric Video</h1>
            <p className="text-sm" style={{ color: "#888" }}>
              Enter your email so we can deliver your video.
            </p>
          </div>

          {/* Feature pills */}
          <div className="flex flex-wrap justify-center gap-2">
            {["Cinematic AI backgrounds", "5 animation styles", "Download instantly"].map(f => (
              <span
                key={f}
          ...
          <p className="text-center text-xs" style={{ color: "#555" }}>
            No account required. We&apos;ll email your video when it&apos;s ready.
          </p>
        </div>
      </div>
    </div>
