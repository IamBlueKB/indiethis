...
/**
 * GateScreen — shown to non-subscribers before the cover art creation wizard.
 *
 * Collects email (+ optional name) via form or Google OAuth.
 * On submit: sets `indiethis_guest_cover_art` cookie (7-day) then reloads.
 */

import { useState }  from "react";
import { useRouter } from "next/navigation";
import { signIn }    from "next-auth/react";
import { Image, Mail, User, ArrowRight, Music2, Sparkles } from "lucide-react";
...
export default function GateScreen() {
  ...
  async function handleContinue(e: React.FormEvent) {
    ...
    try {
      ...
      document.cookie = `indiethis_guest_cover_art=${encodeURIComponent(payload)}; max-age=604800; path=/; SameSite=Lax`;
      router.refresh();
    } catch {
  ...
  function handleGoogle() {
    signIn("google", { callbackUrl: "/cover-art" });
  }

  function handleFacebook() {
    signIn("facebook", { callbackUrl: "/cover-art" });
  }
    ...
    >
      ...
      >
        <a href="/cover-art" className="flex items-center gap-2 no-underline">
          ...
          >
            <Image size={14} style={{ color: "#D4A843" }} />
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-black text-white">Create Your Cover Art</h1>
            <p className="text-sm" style={{ color: "#888" }}>
              Enter your email so we can save your generated artwork.
            </p>
          </div>

          {/* Feature pills */}
          <div className="flex flex-wrap justify-center gap-2">
            {["4–8 AI variations", "Album cover style", "Download instantly"].map(f => (
              <span
                key={f}
          ...
          <form onSubmit={handleContinue} className="space-y-3">
            <div className="relative">
              <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: "#555" }} />
              <input
                type="email"
            ...
            <div className="relative">
              <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: "#555" }} />
              <input
                type="text"
            ...
            >
              {submitting ? "Starting…" : <><span>Continue</span> <ArrowRight size={15} /></>}
            </button>
          </form>

          <p className="text-center text-xs" style={{ color: "#555" }}>
            No account required. We&apos;ll save your artwork so you can download it anytime.
          </p>
          ...
          <div className="text-center">
            ...
            >
              <Music2 size={12} />
              Explore IndieThis artists &rarr;
            </a>
          </div>
        </div>
      </div>
