...
import { Plus, Tag, RefreshCw, X, Check, Copy, ChevronDown } from "lucide-react";
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr
                key={code.id}
                className="border-t transition-colors hover:bg-card/50"
                style={{ borderColor: "var(--border)" }}
              >
...
function CreateCodeModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const [metadata, setMetadata] = useState('{"art":10,"master":3,"video":1}');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (type === "AI_BUNDLE") {
      try { body.metadata = JSON.parse(metadata); } catch { setError("Metadata must be valid JSON."); setLoading(false); return; }
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
      <div className="w-full max-w-lg rounded-2xl border p-6 space-y-5" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
        <div className="flex items-center justify-between">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {type === "AI_BUNDLE" && (
            <div className="space-y-1">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Credits Breakdown (JSON)</label>
              <textarea value={metadata} onChange={e => setMetadata(e.target.value)} className={inputStyle} rows={3} placeholder='{"art":10,"master":3,"video":1}' />
              <p className="text-xs text-muted-foreground">Keys: art, master, video, lyric, aar, press</p>
            </div>
          )}
