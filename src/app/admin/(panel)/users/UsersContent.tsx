...
export default function AdminUsersPage({ viewOnly = false }: { viewOnly?: boolean }) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      {viewOnly && <AdminViewOnlyBanner page="users" />}
      <div className="flex items-center justify-between">
        <div>
        ...
        {loading ? (
          ...
          data.users.map((u) => (
            ...
            >
              ...
              {viewOnly ? <span /> : <UserActions user={u} onRefresh={(patch) => updateUser(u.id, patch)} />}
            </div>
          ))
        )}
      </div>
