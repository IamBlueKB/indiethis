...
1. Scan each file for signal presence (RMS above -40dB) per 50ms window
2. Compare timestamps where multiple files have signal simultaneously
3. Calculate overlap percentage

| Overlap % | Interpretation | Action |
|-----------|---------------|--------|
| < 20% | Vocal comp — artist wants best parts of each take | Crossfade at overlap points (15ms), use whichever file has higher RMS in overlap region |
...
- Tight: replace with silence, crossfade to close gaps (15ms)
...
### 5B. MULTI-FILE HANDLING PER ROLE
...
### 5F. SECTION-AWARE PROCESSING (Premium/Pro)
...
15ms crossfades between section chunks.
