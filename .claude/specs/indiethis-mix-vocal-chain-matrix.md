...
_Includes review fixes: input gain staging, parallel saturation, per-vocalist de-esser, shimmer replaced with large hall + chorus._

---

## HOW IT WORKS

1. **[REVIEW FIX] INPUT GAIN STAGING:** Normalize ALL stems to -23 LUFS via pyloudnorm BEFORE any chain processing. Without this, chain parameters are meaningless.
2. Claude detects genre (from analysis or user selection)
3. Engine classifies each vocal stem by role (lead, double, ad-lib, backing/harmony, ins/outs)
4. Engine looks up the chain: `CHAIN_MATRIX[genre][role]` → gets specific DSP parameters
5. Pedalboard applies the chain per stem
...
## DE-ESSER NOTE [REVIEW FIX]

All de-esser references in this matrix (e.g., "De-esser moderate") use per-vocalist frequency detection — NOT hardcoded 4-8kHz.

- Female vocals: detect sibilance at 8-12kHz
- Male vocals: detect sibilance at 4-7kHz
- Use high-frequency RMS detector to find the dominant sibilant band per file
- Apply at detected frequency with the strength indicated in the Special column

---

## SATURATION NOTE [REVIEW FIX]

All saturation percentages in this matrix are applied as PARALLEL (wet/dry) blend, not inline. The percentage listed is the wet amount — dry signal is preserved at (100% - wet).

Same pattern applies to heavy compression (ratio > 5:1) — run parallel at 50/50 blend to preserve transients.
...
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | +3dB at 3kHz presence, -2dB at 300Hz mud cut | Ratio 4:1, attack 2ms, release 80ms → stage 2: ratio 2.5:1, attack 15ms, release 200ms | Light tube 3% | Short plate, 15% wet, 0.8s decay | None (delay throws from Claude) | Center | None | De-esser per-vocalist [REVIEW FIX], vocal rider |
...
| **Harmonies** | 120Hz 12dB/oct | Wide presence +2dB at 5kHz, warmth +1dB at 200Hz | Ratio 5:1, flatten dynamics | Light 3% | Plate, 25% wet, 1.2s decay | None | Wide L50/R50 spread | ±8 cents per layer | Blend -6 to -8dB below lead, heavy de-ess per-vocalist |

### R&B

| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | +2dB warm shelf at 200Hz, +2dB silky air at 12kHz, -1.5dB at 400Hz | Ratio 3:1, attack 5ms, release 150ms — gentle, preserve dynamics | Warm tube 2% | Plate, 20% wet, 1.5s decay, high damping | Dotted eighth, subtle, -12dB | Center | None | De-esser gentle per-vocalist, vocal rider |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | Bright presence +3dB at 4-5kHz, air +2dB at 12kHz, gentle warmth +1dB at 200Hz — vocals should be bright and forward | Ratio 3:1, attack 8ms, release 180ms — lighter compression, preserve natural dynamics and energy | Light 2% | Room, 15% wet, 0.6s decay — tight, not washy | Dotted eighth tempo-synced, subtle, -10dB | Center | None | De-esser moderate per-vocalist, vocal rider with wider tolerance (allow more dynamic range) |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | Bright presence +3dB at 4kHz, air +2.5dB at 12kHz, cut mud -2dB at 300Hz | Ratio 4:1, attack 3ms, release 100ms → stage 2: ratio 2:1, attack 20ms, release 250ms | Light tube 3% | Plate, 18% wet, 1.2s decay | Dotted eighth, -10dB, 2 repeats | Center | None | Heavy de-essing per-vocalist, vocal rider tight |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 100Hz 12dB/oct | Aggressive presence +4dB at 3kHz, grit +2dB at 1kHz, cut mud -3dB at 300Hz | Ratio 4:1, attack 2ms, release 60ms — punchy, aggressive | Medium drive 5-8% | Room, 12% wet, 0.5s decay — tight | Slapback, 1 repeat | Center | None | De-esser aggressive per-vocalist, vocal rider |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 100Hz 18dB/oct — steep cut, stay out of bass range | Bright +3dB at 5kHz, air +3dB at 12kHz, aggressive mud cut -3dB at 300Hz | Ratio 5:1, attack 1ms, release 50ms — tight, controlled | Light 3% | Large hall, 25% wet, 2.5s decay — spacious | Ping-pong dotted eighth, 3 repeats, -8dB | Center | None | Heavy de-ess per-vocalist, side-chain duck from kick |
| **Ad-libs** | 200Hz 12dB/oct | Filtered — bandpass 500Hz-4kHz for processed feel | Ratio 6:1, squash | Heavy 12%, bitcrusher-style if available | Large hall + chorus, 30% wet, 3s decay [REVIEW FIX: was shimmer — Pedalboard can't do true shimmer] | Ping-pong eighth, 4 repeats | Wide L40/R40 | None | Blend -6dB, heavily processed, part of the texture |
| **Doubles** | 120Hz 12dB/oct | Match lead | Ratio 5:1 | Medium 5% | Same as lead | None | L30/R30 | ±15 cents | Blend -4dB |
| **Harmonies** | 120Hz 12dB/oct | Ethereal — boost 8kHz+ air, cut mids | Ratio 4:1, smooth | Light 2% | Large hall + chorus, 35% wet, 3s+ decay [REVIEW FIX: was shimmer] | None | Wide L50/R50 | ±5 cents | Pad-like, blend -6dB, part of the atmosphere |

### ACOUSTIC / SINGER-SONGWRITER

| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 6dB/oct — gentle slope, keep warmth | Gentle air +1.5dB at 10kHz, warmth +1dB at 200Hz — minimal sculpting | Ratio 2:1, attack 15ms, release 250ms — barely there, preserve dynamics | None — keep it clean | Room or small hall, 12% wet, 1s decay — intimate | None or very subtle eighth, -15dB | Center | None | Light de-ess only per-vocalist, vocal rider with wide tolerance |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | Bright presence +3dB at 4kHz, warmth +2dB at 200Hz — warm and present | Ratio 3:1, attack 5ms, release 150ms | Light tube 3% | Plate, 20% wet, 1.5s decay | Dotted eighth, tempo-synced, -10dB | Center | None | Moderate de-ess per-vocalist, vocal rider |
...
| Role | High-pass | EQ | Compression | Saturation | Reverb | Delay | Pan | Detune | Special |
|------|-----------|-----|-------------|------------|--------|-------|-----|--------|---------|
| **Lead** | 80Hz 12dB/oct | Natural presence +2dB at 3kHz, warmth +1.5dB at 200Hz, air +1dB at 10kHz | Ratio 3:1, attack 10ms, release 200ms — natural, not squashed | Light tape 2% | Room/plate, 15% wet, 1s decay — natural space | Slapback (classic country), 1 repeat, -8dB | Center | None | Moderate de-ess per-vocalist, vocal rider |
...
- Lead: add large hall + chorus reverb layer [REVIEW FIX: was shimmer — not available in Pedalboard]
...
### INPUT GAIN STAGING [REVIEW FIX]

Before the chain matrix is applied, ALL stems must be normalized to -23 LUFS via pyloudnorm. This happens BEFORE `get_chain_for_stem` is called.

```python
import pyloudnorm as pyln

def normalize_input(audio, sr, target_lufs=-23.0):
    meter = pyln.Meter(sr)
    loudness = meter.integrated_loudness(audio)
    return pyln.normalize.loudness(audio, loudness, target_lufs)

# Call on every stem BEFORE any chain processing
audio = normalize_input(audio, sr)
chain = get_chain_for_stem(genre, role, vocal_style_preset)
# ... apply chain to normalized audio
```
...
def get_chain_for_stem(genre, role, vocal_style_preset=None):
    ...
    if vocal_style_preset == "clean_natural":
        ...
        chain["air_boost"] = 2
        chain["reverb_type"] = "large_hall_chorus"  # [REVIEW FIX: was shimmer]
        if "bandpass" in chain:
            del chain["bandpass"]
...
## REVIEW FIXES IN THIS DOCUMENT

| # | Fix | Where |
|---|-----|-------|
| 1 | Input gain staging — normalize to -23 LUFS before chain | HOW IT WORKS step 1, IMPLEMENTATION section |
| 2 | Parallel saturation mix control — wet/dry on all saturation + heavy compression | SATURATION NOTE section, code examples |
| 3 | De-esser per-vocalist detection — not hardcoded 4-8kHz | DE-ESSER NOTE section, all genre tables updated |
| 4 | Shimmer replaced with large hall + chorus — Pedalboard can't do true shimmer | Electronic ad-libs, Electronic harmonies, Airy preset |
