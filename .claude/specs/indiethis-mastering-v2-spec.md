...
## 7. DIRECTION DROPDOWNS (REPLACES TEXTAREA)

**Problem:** The Direction field is a blank textarea with stem-level placeholder text ("More reverb on the chorus. Punchier kick.") that doesn't apply to stereo 2-track mastering. Most artists don't know what to type.

**Fix:** Replace the textarea with two dropdown menus. Artist picks one from each (both optional). The engine uses the selections to adjust processing chains.

**Dropdown 1 — Vibe Direction:**

| Option | What the engine does |
|--------|---------------------|
| Warm & full | Low shelf boost, gentle high roll-off, light compression |
| Bright & crisp | High shelf boost, mid-high presence, clean limiting |
| Punchy & loud | Mid presence boost, aggressive compression, pushed LUFS |
| Lo-fi / vintage | Subtle saturation, rolled highs, relaxed dynamics |
| Natural / transparent | Minimal processing, preserve original character, clean LUFS normalization |

**Dropdown 2 — Platform Ready:**

| Option | Target LUFS | What the engine does |
|--------|------------|---------------------|
| Spotify ready | -14 LUFS | Standard normalization, balanced for Spotify's volume leveling |
| Apple Music ready | -16 LUFS | Slightly quieter target, more dynamic range preserved |
| YouTube ready | -14 LUFS | Optimized for video playback, slightly brighter |
| Club / DJ ready | -8 to -10 LUFS | Louder, more compression, punchy low end for club systems |
| Radio ready | -12 LUFS | Broadcast standard, controlled dynamics, clear vocals |
| TikTok / Reels ready | -14 LUFS | Short-form optimized, bright and attention-grabbing, strong high end |

**Default:** If artist picks nothing, engine uses Natural / transparent + Spotify ready.

**Custom option (Premium/Pro only):** Below the two dropdowns, a "Custom direction" textarea appears for Premium/Pro tiers only. Placeholder: "Add specific notes — e.g. more low end, wider stereo, brighter vocals." This is in addition to the dropdowns, not a replacement.

**For stems uploads:** Add a third dropdown for mix-level direction:

| Option | What the engine does |
|--------|---------------------|
| Vocal forward | Boost vocal presence, duck competing frequencies |
| Bass heavy | Sub boost, tighten kick, side-chain compression |
| Balanced mix | Even stem levels, gentle processing across all stems |
| Drum focused | Punch up kicks and snares, tighten cymbals |

**Logging:** Store both selections on the `MasteringJob`:

| Field | Type |
|-------|------|
| `vibeDirection` | String (enum) |
| `platformTarget` | String (enum) |
| `customDirection` | String (nullable, Premium/Pro only) |
| `mixDirection` | String (nullable, stems mode only) |

**Tier access:**

| Tier | Vibe dropdown | Platform dropdown | Custom textarea | Mix dropdown (stems) |
|------|--------------|-------------------|----------------|---------------------|
| Standard ($7.99) | Yes | Yes | No | Yes |
| Premium ($14.99) | Yes | Yes | Yes | Yes |
| Pro ($24.99) | Yes | Yes | Yes | Yes |

All tiers get both dropdowns. Custom textarea is the Premium/Pro upsell.
...
2. Direction dropdowns (vibe + platform ready) — replaces textarea
